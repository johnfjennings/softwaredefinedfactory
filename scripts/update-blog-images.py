...
BASE = "c:/JohnJennings/_Projects/ClaudePro/softwaredefinedfactory/src/content/blog"
...
for filename, data in posts.items():
    ...
    if 'coverImage:' in content:
        content = re.sub(r'coverImage: ".*?"', f'coverImage: "{data["cover"]}"', content)
    else:
        # Insert coverImage before the closing --- of frontmatter
        content = re.sub(
            r'(\ntags: \[.*?\])\n---',
    ...
    comment = f'<!-- hero-image-prompt: {data["prompt"]} -->'
