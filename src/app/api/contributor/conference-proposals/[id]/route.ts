...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing } = await supabase.from("conference_proposals").select("submitted_by, status").eq("id", id).single()
  ...
  const { error } = await supabase.from("conference_proposals").update({ ...body, updated_at: new Date().toISOString() }).eq("id", id)
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 })
  if (existing.submitted_by !== auth.user.id) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
  const { error } = await supabase.from("conference_proposals").delete().eq("id", id)
