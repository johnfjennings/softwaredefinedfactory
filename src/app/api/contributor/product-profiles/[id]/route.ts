...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing } = await supabase.from("product_profiles").select("submitted_by, status").eq("id", id).single()
  ...
  const { error } = await supabase.from("product_profiles").update({ ...body, updated_at: new Date().toISOString() }).eq("id", id)
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabase.from("product_profiles").delete().eq("id", id)
